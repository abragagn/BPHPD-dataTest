...
void cutFlow(TString fileName = "ntu2018Dv2.root", TCut cutEvt = "")
{
    ...
    TH1 *hNoCut     = new TH1F("hNoCut",    "No Cuts",   nBins_, min_, max_);
    TH1 *hJpsiCut   = new TH1F("hJpsiCut",  "Jpsi Cuts", nBins_, min_, max_);
    TH1 *hPhiCut    = new TH1F("hPhiCut",   "Phi Cuts",  nBins_, min_, max_);
    TH1 *hBsCut     = new TH1F("hBsCut",    "Bs Cuts",   nBins_, min_, max_);
    TH1 *hCtCut     = new TH1F("hCtCut",    "Ct Cuts",   nBins_, min_, max_);


    TCut cNoCut = cutEvt;

    TCut cJpsiCut = cNoCut;
    cJpsiCut += "fabs(jpsiMass - 3.097) < 0.15";
    cJpsiCut += "jpsiPt>7.0";
    cJpsiCut += "muPlus_Pt > 4.0 && muMinus_Pt > 4.0";
    cJpsiCut += "fabs(muPlus_Eta) < 2.2 && fabs(muMinus_Eta) < 2.2";

    TCut cPhiCut = cJpsiCut;
